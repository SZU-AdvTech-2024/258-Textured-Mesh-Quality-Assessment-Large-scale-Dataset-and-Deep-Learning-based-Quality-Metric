...
class Trainer():
    ...
    def clamp_weights(self):
        for module in self.net.modules():
            if hasattr(module, 'weight') and isinstance(module, torch.nn.Conv2d) and module.kernel_size == (1, 1):
    ...
    def forward_train(self): # run forward pass
        ...
        # self.var_judge is the same as self.input_judge调整输入向量的形状
        ...
        #计算与每个唯一 stimulus 相关的 judge 的平均值
    ...
    def backward_train(self):
        ...
        #torch.mean is useless since we have only one "loss_total" value/batch, and this function is excecuted per batch
...
def Testset_DSIS(data_loader, func, funcLoss = None, name=''): #added by yana
    ...
    mos_predicteds_trans = [tensor.item() for tensor in MOSpredicteds]  # 提取值
    mos_trans = [tensor.item() for tensor in MOSs]  # 提取值
    srocc = stats.spearmanr(mos_predicteds_trans, mos_trans)[0]
    # srocc = stats.spearmanr(MOSpredicteds.numpy(), MOS.numpy())[0]
    #报错can't convert cuda:0 device type tensor to numpy. Use Tensor.cpu() to copy the tensor to host memory first.
