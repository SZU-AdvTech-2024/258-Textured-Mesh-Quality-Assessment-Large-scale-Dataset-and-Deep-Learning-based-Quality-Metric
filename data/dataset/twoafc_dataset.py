...
class TwoAFCDataset(BaseDataset):
    def initialize(self, dataroots, load_size=64, Trainset = False, maxNbPatches = 205):
        ...
        if(Trainset):
            root_judges = '.\\dataset\\judge_trainingset'
        else:
            root_judges = '.\\dataset\\judge_testset'
        nbiteration = 1
        stimuliId = 0 
        ...
        if(Trainset):
            ...
            for datafile in dataroots:
                ...
                shuffledfileName = '.\\dataset\\'+ 'Trainset_shuffled_' + str(count_inputFile) +'.csv'
        ...
        '''
        ref_paths:所有的参考图像的位置
        p0_paths:所有的扭曲图像的位置
        judge_paths:所有的npy文件的位置
        stimuliId:扭曲模型的ID，用于区分不同的扭曲模型
        '''
        self.ref_paths = []
        self.p0_paths = []
        self.judge_paths = []
        ...
        print('dataroot:',dataroots)
        for datafile in dataroots:
            with open(datafile) as csv_file:
                csv_reader = csv.reader(csv_file, delimiter=',')
                line_count = 0
                for row in csv_reader:
                    #循环Trainset_shuffled_1.csv中的模型
                    if line_count == 0:
                        ...
                        nbPatches = nbPatchesVP1
                        # For "view independent scenario": nbPatches = Total Nb Patches Per Model = nbPatchesVP1 + nbPatchesVP2 + nbPatchesVP3 +nbPatchesVP4
                        ...
                        for itr in range(1, nbiteration+1):
                            ...
                            # complete with random patches to reach the max nb of patches随机挑选凑够150片
        ...
        print('Total nb of patches to load: %.1f' %len(self.p0_paths))
        #扭曲模型的补丁的数量 must be equal to maxNbPatches * nb rows * nb iterations
        ...
        NbuniquestimulusID = len(uniquestimulusID)#扭曲模型数量
        ...
        self.transform = transforms.Compose(transform_list)#变换
